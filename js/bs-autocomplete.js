Vue.component('bs-autocomplete', {
	...
	template: `
        <div style="position: relative" ref="outer_div">
            <div :class="select_class" @click.prevent="toggle_list" v-bind="$attrs" @keydown.38="press_up" @keydown.40="press_down" @keypress.enter.stop.prevent="press_enter">
                <template v-if="selected_item" >
                    <slot name="display_text" v-bind="{item:selected_item}">{{ get_item_text(selected_item) }}</slot>
                </template>
            </div>
			<input style="opacity: 0; width: 0; height: 0; border: none; position: absolute; pointer-events: none" :name="$attrs.name" :required="$attrs.required" :value="value" >
